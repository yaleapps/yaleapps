...
export const useFormStore = defineStore("favorites", () => {
	...
	async function fetchAbbreviatedCatalog() {
	...
	async function submitForm() {
		try {
			const { error } = await supabase.from("UserCourse").insert({
				...
				selected_favorite_lecture_courses: selectedFavoriteLectureCourses.value,
				selected_favorite_seminar_courses: selectedFavoriteSeminarCourses.value,
